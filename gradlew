...
#       temporary focusing, currenturrently not supported.
#
...
# Prefixing spaces to argument list before passing to java is bad practice.
# We only do it for arguments that start with a dash, to work around
# a problem with the JVM's command line argument parsing.
# See: https://github.com/gradle/gradle/pull/35
collect_all_args () {
    for arg do
        shift
        case $arg in
...
# Stop when "xeli" is set and "set -e" is active.
